...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO pass_data (full_name, roll_no, department, room_no, reason, student_phone, parent_phone, going, addres, outgoing_date, outgoing_time, incoming_date, incoming_time,Email,hostel) 
                                VALUES (:full_name, :roll_no, :department, :room_no, :reason, :student_phone, :parent_phone, :going, :addres, :outgoing_date, :outgoing_time, :incoming_date, :incoming_time,:Email,:hostel)");
        ...
        $stmt->bindParam(':student_phone', $_POST["student_phone"]);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $message .= file_get_contents('http://localhost:8080/allow_deny.html');
...
$mail->Port = 587; // TCP port to connect to
        ...
        // Send email
        $mail->send();
        header("Location: http://localhost:8080/success.html");
    } catch (Exception $e) {
